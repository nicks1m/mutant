...
class Autoencoder:
    ...
    def _add_decoder_input(self):
        return Input(shape = self.latent_space_dim, name="deocder_input")

    def _add_dense_layer(self,decoder_input):
        num_neurons = np.prod(self._shape_before_bottleneck) # multiply items in array
    ...
    def _add_conv_transpose_layers(self,x):
        ...
        # loop thru all conv layers in reverse order and stop at the first layer
        for layer_index in reversed(range(1, self._num_conv_layers)):
            # [0, 1, 2] -> [2, 1] we start from second index
    ...
    def _add_bottleneck(self, x):
        ...
        self._shape_before_bottleneck = K.int_shape(x)[1:] # ignoring first dimension
        x = Flatten()(x)
